...
export default function DataTable({ 
  ...
  const aplicarRegrasAoValor = (valor, regras = []) => {
    if (!regras || regras.length === 0) return true;
    const valNum = parseFloat(valor);
    const valStr = valor === null || valor === undefined || String(valor).trim() === "" ? "##NULL##" : String(valor);
    const numFilters = regras.filter(f => f.startsWith(">=|") || f.startsWith("<=|"));
    const exactFilters = regras.filter(f => !f.startsWith(">=|") && !f.startsWith("<=|"));
    if (numFilters.length > 0) {
      if (isNaN(valNum)) return false;
      for (const nf of numFilters) {
        if (nf.startsWith(">=|") && valNum < parseFloat(nf.split(">=|")[1])) return false;
        if (nf.startsWith("<=|") && valNum > parseFloat(nf.split("<=|")[1])) return false;
      }
    }
    if (exactFilters.length > 0 && !exactFilters.includes(valStr)) return false;
    return true;
  };

  const obterFiltrosCascata = (coluna) => {
    const ordem = colunasVisiveis.length ? colunasVisiveis : colunasTabela;
    const indiceAtual = ordem.indexOf(coluna);
    if (indiceAtual <= 0) return {};
    const anteriores = {};
    ordem.slice(0, indiceAtual).forEach(col => {
      if (filtrosGlobais[col]?.length) anteriores[col] = filtrosGlobais[col];
    });
    return anteriores;
  };
  ...
  const abrirMenuExcel = async (coluna, e) => {
    ...
    const filtrosAnteriores = obterFiltrosCascata(coluna);

    if (onFetchColumnOptions && (Object.keys(filtrosAnteriores).length === 0 || onFetchColumnOptions.length >= 2)) {
      setLoadingOpcoes(true);
      try {
        const opcoes = await onFetchColumnOptions(coluna, filtrosAnteriores, filtrosGlobais);
  ...
  const opcoesFiltroAtual = useMemo(() => {
    ...
    const filtrosAnteriores = obterFiltrosCascata(coluna);
    const temCascata = Object.keys(filtrosAnteriores).length > 0;
    let baseOpcoes = [];

    if (temCascata) {
      const dadosBase = data.filter(item =>
        Object.entries(filtrosAnteriores).every(([col, regras]) => aplicarRegrasAoValor(item[col], regras))
      );
      const vistos = new Set();
      dadosBase.forEach(item => {
        const val = item[coluna];
        const isNull = val === null || val === undefined || String(val).trim() === "";
        const chave = isNull ? "##NULL##" : String(val);
        const exibicao = isNull ? "-" : formatarDado(val, coluna);
        if (!vistos.has(chave)) {
          vistos.add(chave);
      ...
      if (opcoesBancoColuna.length > 0 && onFetchColumnOptions?.length >= 2) {
    ...
    return baseOpcoes.filter(op => op.exibicao.toLowerCase().includes(termoLower))
      .sort((a, b) => a.exibicao.localeCompare(b.exibicao));
  }, [opcoesBancoColuna, menuAtivo, data, termoBuscaFiltro, filtrosGlobais, colunasVisiveis, colunasTabela, onFetchColumnOptions]);
          ...
          <thead>
            ...
            <tr>
              ...
              {colunasVisiveis.map((col, idx) => {
                ...
                const filtroAtivo = filtrosGlobais[col] && filtrosGlobais[col].length > 0;
                const ativo = filtroAtivo || ordenacao.coluna === col;
                    ...
                    <div className="th-cell-container" onClick={(e) => abrirMenuExcel(col, e)}>
                      ...
                      <div className="th-actions-group">
                        ...
                        <button 
                          ...
                          title={filtroAtivo ? "Filtro aplicado nesta coluna" : "Filtrar coluna"}

                        >

                          <ChevronDown size={14} style={filtroAtivo ? { color: '#9a6700', strokeWidth: 2.8 } : undefined} />
