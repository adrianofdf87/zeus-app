...
export default function Producao() {
  ...
  const [ordenacaoTabela1, setOrdenacaoTabela1] = useState({
    campo: "soma_valor_proj",
    direcao: "desc"
  });

  const [ordenacaoTabela2, setOrdenacaoTabela2] = useState({
    campo: "num_os",
    direcao: "asc"
  });
  ...
  useEffect(() => {
    const handleClickFora = (e) => {
      if (
        dropdownColunaRef.current &&
        !dropdownColunaRef.current.contains(e.target)
      ) {
        setDropdownColunaAberto(false);
      }

      if (
        dropdownValorRef.current &&
        !dropdownValorRef.current.contains(e.target)
      ) {
  ...
  const carregarTodosDadosProdutividade = async () => {
    try {
      ...
      console.error(
        "Erro ao carregar view_dados_produtividade:",
        error
      );
  ...
  const adicionarFiltroDinamico = () => {
    ...
    setFiltrosAtivos(prev => {
      const novosFiltros = {
        ...prev,
        [tipoFiltroAtual]: [...valoresSelecionadosTemp]
      };
  ...
  const removerFiltroItem = (
    ...
    setFiltrosAtivos(prev => {
      ...
      if (novosFiltros[campo]) {
        novosFiltros[campo] =
          novosFiltros[campo].filter(
            v => v !== valorParaRemover
          );
  ...
  const processarDados = (
    ...
    dados.forEach(item => {
      ...
      Object.keys(filtros).forEach(campo => {
        ...
        if (
          ...
          const temAlgumMesPermitido =
            valoresPermitidos.some(
              mes =>
                valItem &&
                valItem.hasOwnProperty(mes) &&
                Number(valItem[mes]) > 0
            );
          ...
          const atendeData =
            valoresPermitidos.some(v =>
              String(valItem).startsWith(v)
            );
      ...
      if (
        cruzadoNumOs &&
        (
          item.num_os !== cruzadoNumOs &&
          item.ordem_servico !== cruzadoNumOs
        )
      ) {
        atendeTodos = false;
      }
      ...
      if (atendeTodos) {
        ...
        if (
          filtros.meses &&
          Array.isArray(filtros.meses) &&
          filtros.meses.length > 0 &&
          item.meses &&
          typeof item.meses === "object"
        ) {
          let somaProdMesesFiltrados = 0;

          filtros.meses.forEach(mes => {
            if (
              item.meses[mes] !== undefined
            ) {
              somaProdMesesFiltrados +=
                Number(item.meses[mes]) || 0;
            }
          });

          itemProcessado.valor_prod =
            somaProdMesesFiltrados;
        }
    ...
    dados.forEach(item => {
      ...
      if (
        ...
        Object.values(item.meses).forEach(
          valMes => {
            totalOs += Number(valMes) || 0;
          }
        );

        somaTotalMesesPorOs[numOs] =
          totalOs;
      } else {
        somaTotalMesesPorOs[numOs] =
          Number(item.valor_prod) || 0;
      }
    });
    ...
    filtrados.forEach(item => {
      ...
      const valProj =
        Number(item.valor_proj) || 0;

      const valProd =
        Number(item.valor_prod) || 0;

      const valProdTotalGeral =
        somaTotalMesesPorOs[numOs] !== undefined
          ? somaTotalMesesPorOs[numOs]
          : valProd;

      const valFatu =
        Number(item.valor_fatu) || 0;
      ...
      if (numOs) {
        agrupadoTipo[tipo].numOsSet.add(numOs);
      }

      agrupadoTipo[tipo].soma_valor_proj +=
        valProj;

      agrupadoTipo[tipo].soma_valor_prod +=
        valProd;

      agrupadoTipo[tipo].soma_valor_fatu +=
        valFatu;
      ...
      if (!agrupadoNumOs[numOs]) {
        ...
        agrupadoNumOs[numOs].valor_proj +=
          valProj;

        agrupadoNumOs[numOs].valor_prod +=
          valProd;

        agrupadoNumOs[numOs].valor_fatu +=
          valFatu;
      }
    });
    ...
    const resultadoTabela1 =
      Object.values(agrupadoTipo).map(
        grupo => {
          const percProj =
            somaGeralProj > 0
              ? (grupo.soma_valor_proj /
                  somaGeralProj) *
                100
              : 0;
    ...
    const resultadoTabela2 =
      Object.values(agrupadoNumOs).map(
        item => {
          const prodXproj =
            item.valor_proj > 0
              ? (item.valor_prod_total /
                  item.valor_proj) *
                100
              : 0;
  ...
  const handleLinhaTabela1Click = (
    tipoOs
  ) => {
    const novoFiltro =
      filtroCruzadoTipoOs === tipoOs
        ? null
        : tipoOs;
  ...
  const handleLinhaTabela2Click = (
    numOs
  ) => {
    const novoFiltro =
      filtroCruzadoNumOs === numOs
        ? null
        : numOs;
  ...
  const dadosTabela1Ordenados =
    ordenarDados(
      dadosProcessadosTabela,
      ordenacaoTabela1.campo,
      ordenacaoTabela1.direcao
    );

  const dadosTabela2Ordenados =
    ordenarDados(
      dadosIndividuaisNumOs,
      ordenacaoTabela2.campo,
      ordenacaoTabela2.direcao
    );

  const alternarOrdenacaoTabela1 =
    campo => {
  ...
  const alternarOrdenacaoTabela2 =
    campo => {
   ...
   * IGUAL OU ACIMA DE 100% = VERDE
   *
   * A barra continua limitada visualmente a 100%.
   * O número, porém, mostra o percentual real.
   */
  const renderPercentual = (
    ...
    const atingiuMeta = numero >= 100;
    const cor = atingiuMeta
      ? "#16a34a"
      : "#dc2626";
  ...
  const colunasFiltradasPelaBusca =
    colunasDisponiveis.filter(col =>
      col &&
      col
        .toLowerCase()
        .includes(
          termoPesquisaColuna.toLowerCase()
        )
    );

  const valoresFiltradosPelaBusca =
    valoresColunaAtual.filter(val =>
      val &&
      String(val)
        .toLowerCase()
        .includes(
          termoPesquisaValor.toLowerCase()
        )
    );
  ...
  const selecionarTodosValoresVisiveis =
    () => {
      const todosVisiveis =
        valoresFiltradosPelaBusca;

      const todosJaEstaoSelecionados =
        todosVisiveis.every(v =>
          valoresSelecionadosTemp.includes(v)
        );
      ...
      if (todosJaEstaoSelecionados) {
        setValoresSelecionadosTemp(
          prev =>
            prev.filter(
              v =>
                !todosVisiveis.includes(v)
            )
        );
      } else {
        const combinados = new Set([
          ...valoresSelecionadosTemp,
          ...todosVisiveis
        ]);

        setValoresSelecionadosTemp(
          Array.from(combinados)
        );
      }
    };
  ...
  const temFiltroAtivo =
    ...
    filtroCruzadoTipoOs !== null ||
    filtroCruzadoNumOs !== null;
  ...
  return (
    /*
     * IMPORTANTE:
     * O componente inteiro ocupa a altura disponível.
     * O overflow fica escondido aqui.
     * Assim o scroll NÃO começa no topo da página.
     */
    <div
      style={{
        width: "100%",
    ...
    >
      ...
      {/* CABEÇALHO E FILTROS - FORA DO SCROLL */}
      <div
        style={{
          flexShrink: 0,
          position: "relative",
      ...
      >
          ...
          >
            ...
            <div>
              ...
              >
                ...
                de OS (view_dados_produtividade).
              </p>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div
                  style={{
                    ...
                    cursor: "pointer",
                    fontSize: "12px",
                    color: tipoFiltroAtual
                      ? "#1e293b"
                      : "#64748b"
                  }}
                  onClick={() =>
                    setDropdownColunaAberto(
                      prev => !prev
                    )
                  }
                >
              ...
              >
                <div
                  ...
                  onClick={() =>
                    setDropdownValorAberto(
                      prev => !prev
                    )
                  }
                >
                  <span>
                    {valoresSelecionadosTemp.length ===
                    0
                ...
                {dropdownValorAberto && (
                  ...
                  >
                    ...
                    >
                      <span
                        onClick={
                          selecionarTodosValoresVisiveis
                        }
                      >
                        Selecionar Visíveis
                      </span>
                        ...
                        valoresFiltradosPelaBusca.map(
                          val => (
                            <label
                              ...
                              style={{
                                ...
                                cursor:
                                  "pointer",
                                background:
                                  valoresSelecionadosTemp.includes(
                                    val
                                  )
                                    ? "#e0f2fe"
                                    : "transparent"
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={valoresSelecionadosTemp.includes(
                                  val
                                )}
                                onChange={() =>
                                  toggleValorTemp(
              ...
              {temFiltroAtivo && (
                <button
                  type="button"
                  className="filter-btn-limpar-todos"
                  style={alturaUnificadaEstilo}
                  onClick={
                    limparTodosFiltros
                  }
            ...
            {/* CHIPS */}
            <div
              className="filter-badges-container"
              style={{
                display: "flex",
                gap: "3px",
                flexWrap: "wrap",
      ...
      {/* 
        ...
        CONTEÚDO COM SCROLL
        ==========================================================
        ESTE É O ÚNICO ELEMENTO QUE POSSUI O SCROLL PRINCIPAL.
        Portanto o scroll começa ABAIXO dos filtros.
      */}
      <div
        style={{
          flex: 1,
      ...
      >
        ...
        >
          ...
          0 ? (
            ...
            >
              <table
                className="tabela-apoio-estilizada"
                style={{
                  width: "100%"
                }}
              >
                <thead>
                  <tr>
                    <th
                      onClick={() =>
                        alternarOrdenacaoTabela1(
                          "tipo_os"
                        )
                      }
                      style={getEstiloCabecalho(
                        ordenacaoTabela1.campo,
                        "tipo_os"
                      )}
                    >
                    ...
                    (item, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {formatarMoeda(
                              item.soma_valor_proj
                            )}
                          </td>

                          <td>
                            {renderPercentual(
                              item.perc_valor_proj
                            )}
                          </td>
                          ...
                          >
                            {formatarMoeda(
                              item.soma_valor_prod
                            )}
                          </td>

                          <td>
                            {renderPercentual(
                              item.perc_valor_prod
                            )}
                          </td>
                          ...
                          >
                            {formatarMoeda(
                              item.soma_valor_fatu
                            )}
                          </td>

                          <td>
                            {renderPercentual(
                              item.perc_valor_fatu
                            )}
                          </td>
                        </tr>
                      );
        ...
        >
          ...
          >
            ...
            >
              Detalhamento por Ordem de
              Serviço (Num OS Individuais
              com Scroll Interno)
            </h3>
          </div>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      ["pep", "PEP"],
                      ["status", "Status"],
                      ["valor_proj", "Valor Projetado"],
                      ["valor_prod_total", "Produzido Total"],
                      ["valor_prod", "Valor Produzido (Mês)"],
                      ["valor_fatu", "Valor Faturado"],
                      ["prod_x_proj", "%ProdXProj"],
                      ["fatu_x_prod", "%FatuXProd"],
                      ["fatu_x_proj", "%FatuXProj"]
                    ].map(
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    {formatarMoeda(
                      totaisGerais.valorProdTotalGeral
                    )}
                  </span>
                </div>
