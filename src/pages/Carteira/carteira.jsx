...
import { Briefcase, Map, CheckCircle, Calendar, Flag, FileText, Package, Wrench, Paperclip, Upload, PlusCircle, Edit3, Trash2, RefreshCw, Download, FilterX, ChevronRight, ChevronLeft, Search } from "lucide-react";
...
export default function Carteira() {
  ...
  const perfilUsuario = String(sessaoUsuario.Perfil || sessaoUsuario.perfil || sessaoUsuario.cargo || sessaoUsuario.tipo || sessaoUsuario.role || '').trim().toLowerCase();
  ...
  const [totais, setTotais] = useState({ equipes: 0, referencia: 0, servicos: 0, percentual: 0, ticketMedio: 0, qtdObras: 0, poste: 0, kmmt: 0, kmbt: 0, trafo: 0, clientes: 0 });
  ...
  useEffect(() => {
    if (!getEl('carteira-import-styles')) {
      ...
      style.innerHTML = `
        ...
        @keyframes loadingDots { 0% { content: ''; } 25% { content: '.'; } 50% { content: '..'; } 75% { content: '...'; } }
        .loading-dots::after { display: inline-block; animation: loadingDots 1.5s infinite steps(4,end); content: ''; width: 16px; text-align: left; }
      `;
  ...
  const carregarEstrutura = async () => {
    try {
      ...
      if (!error && Array.isArray(data)) setEstruturaTabela(data);
  ...
  useEffect(() => {
    ...
    const handleResize = () => { verificarScrollAbas(); verificarScrollKpis(); };
  ...
  const carregarCarteira = useCallback(async (isBackground = false) => {
    setLoading(true);
    if (!isBackground) setListaCarteiraGlobal([]);
    ...
    try {
      ...
      if (termoBruto.length >= 2) {
        const colunasObrigatorias = [
          'aviso','carteira','filial','contratante','municipio','seccional',
          'area','tipo','tipo_custo','tipo_atividade','prioridade',
          'tipo_rastreio','pep','descricao','descrição','processo',
          'status','observacao','observação'
        ];

        let colunasTexto = estData
          .filter(c => {
            const nome = String(c.nome_coluna || c.column_name || '').trim();
            const tipo = String(c.tipo || c.data_type || c.udt_name || '').toLowerCase();
            return nome && /char|text|string|varchar|citext/.test(tipo);
          })
          .map(c => String(c.nome_coluna || c.column_name).trim());

        colunasTexto = [...new Set([...colunasTexto, ...colunasObrigatorias])]
          .filter(col => /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(col));

        const termoLimpo = termoBruto
          .replace(/[(),]/g, ' ')
          .replace(/\s+/g, ' ')
          .trim()
          .replace(/\\/g, '\\\\')
        ...
        if (termoLimpo.length > 0 && colunasTexto.length > 0) {
      ...
      const { data, count, error } = await query.range(from, from + registrosPorPagina - 1).order('id', { ascending: false });
  ...
  const handleSelectionChange = (ids) => {
    ...
    } else setRegistroSelecionadoObj(null);
  ...
  const handleExcluirAtividade = async () => {
    ...
    const confirmacao = await Swal.fire({
      title:'Deseja Excluir?', text:`A atividade de ID: ${idParaExcluir}`, icon:'warning',
      showCancelButton:true, confirmButtonColor:'#dc2626', cancelButtonColor:'#64748b',
      confirmButtonText:'Sim, excluir', cancelButtonText:'Cancelar'
    });
    if (confirmacao.isConfirmed) {
      try {
        const { data: atividadeObj } = await supabase.from("tabe_cad_carteira").select("id_rastreio").eq('id', idParaExcluir).maybeSingle();
        ...
        if (idRastreio) await supabase.from("tabe_imp_pep_lto").update({ id_atividade:null }).eq('nota', idRastreio);
  ...
  const calcularTotalReferencia = async () => {
    try {
      ...
      if (maxDateData && maxDateData.length > 0) {
        ...
        (data || []).forEach(item => { somaMeta += parseFloat(item.meta || 0); if(item.prefixo) setEquipes.add(String(item.prefixo).trim()); });
      }
  ...
  const getTabStyle = (isActive,isLocked) => ({
    ...
    padding:'8px 14px',fontSize:'0.85rem',fontWeight:isActive?'700':'500',
    color:isActive?'#0284c7':isLocked?'#94a3b8':'#475569',cursor:'pointer',
    display:'inline-flex',alignItems:'center',gap:'6px',borderRadius:'6px 6px 0 0',
    transition:'all 0.2s',whiteSpace:'nowrap',opacity:isLocked?0.6:1,height:'37px'
  });
  ...
  return (
    <div translate="no" style={{padding:'0 15px 10px 15px',height:'100%',display:'flex',flexDirection:'column',boxSizing:'border-box',overflow:'hidden',background:'#f8fafc'}}>
      <style>{`.custom-scroll::-webkit-scrollbar{height:0px;display:none}.custom-scroll{scrollbar-width:none}`}</style>
      ...
      <div style={{position:'sticky',top:0,zIndex:100,background:'#f8fafc',paddingTop:'10px',paddingBottom:'4px',borderBottom:'2px solid #e2e8f0',marginBottom:'14px'}}>
        <div style={{display:'flex',width:'100%',alignItems:'center',position:'relative'}}>
          <button onClick={()=>rolarAbas(-1)} disabled={!podeRolarAbasEsq} style={{...minimalScrollBtnStyle,display:temOverflowAbas?'flex':'none',marginRight:'6px'}}><ChevronLeft size={15}/></button>
          <div style={{display:'flex',gap:'6px',flexShrink:0,background:'#f8fafc',zIndex:3,alignItems:'center'}}>
            {abasFixas.map(tab=>{const Icon=tab.icon;return <button key={tab.id} onClick={()=>mudarAba(tab.id)} style={getTabStyle(abaAtiva===tab.id,false)}><Icon size={14}/>{tab.label}</button>})}
          </div>
          <div ref={abasScrollRef} className="custom-scroll" onScroll={verificarScrollAbas} style={{display:'flex',gap:'6px',overflowX:'auto',flex:1,paddingLeft:'4px',paddingRight:'4px',alignItems:'center'}}>
            {abasMoveis.map(tab=>{const Icon=tab.icon;return <button key={tab.id} onClick={()=>mudarAba(tab.id)} style={getTabStyle(abaAtiva===tab.id,tab.locked)}><Icon size={14}/>{tab.label}</button>})}
          </div>
          <button onClick={()=>rolarAbas(1)} disabled={!podeRolarAbasDir} style={{...minimalScrollBtnStyle,display:temOverflowAbas?'flex':'none',marginLeft:'4px'}}><ChevronRight size={15}/></button>
        </div>
      </div>
        ...
        <div style={{display:'flex',flexDirection:'column',flex:1,minHeight:0,gap:'12px',overflow:'hidden'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px',flexShrink:0,flexWrap:'wrap'}}>
            <div style={{position:'relative',flex:'1',minWidth:'280px'}}>
              ...
              <input type="text" placeholder="Pesquisar geral (ex: DEMANDA DE CLIENTE)..." value={busca} onChange={e=>{setBusca(e.target.value);setPaginaAtual(1)}} onPaste={e=>{e.preventDefault();setBusca(e.clipboardData.getData('text').trim());setPaginaAtual(1)}} style={{width:'100%',padding:'0 12px 0 36px',height:'32px',borderRadius:'6px',border:'1px solid #cbd5e1',outline:'none',fontSize:'0.85rem',background:'#fff',boxSizing:'border-box'}}/>
            </div>
            <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
              {temFiltroAtivo && <button style={{...btnStyle,color:'#dc2626',minWidth:'32px',width:'32px',padding:'0',justifyContent:'center'}} title="Limpar filtros e busca" onClick={limparTodosFiltros}><FilterX size={14}/></button>}
              ...
              {podeExcluir && <button style={{...btnStyle,color:'#dc2626',borderColor:'#fca5a5',opacity:idsSelecionados.length!==1?0.4:1}} disabled={idsSelecionados.length!==1} onClick={handleExcluirAtividade}><Trash2 size={14}/> Excluir</button>}
          ...
          <div style={{background:'#f8fafc',border:'1px solid #cbd5e1',borderRadius:'8px',padding:'6px 16px',flexShrink:0}}>
            <div style={{display:'flex',alignItems:'center',position:'relative'}}>
              <button onClick={()=>rolarKpis(-1)} disabled={!podeRolarKpisEsq} style={{...minimalScrollBtnStyle,display:temOverflowKpis?'flex':'none',marginRight:'8px'}}><ChevronLeft size={15}/></button>
              <div style={{display:'grid',gridTemplateColumns:'repeat(5,minmax(80px,1fr))',gap:'8px',textAlign:'center',flexShrink:0,paddingRight:'12px',borderRight:'2px solid #cbd5e1'}}>
                <div><span style={kpiLabel}>Qtd Equipes</span><strong style={kpiValueBlue}>{totais.equipes}</strong></div>
                <div><span style={kpiLabel}>Valor Referência</span><strong style={kpiValueBlue}>{new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL',maximumFractionDigits:0}).format(totais.referencia)}</strong></div>
                <div><span style={kpiLabel}>Valor Carteira</span><strong style={kpiValueBlue}>{new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL',maximumFractionDigits:0}).format(totais.servicos)}</strong></div>
                <div><span style={kpiLabel}>% (Carteira)</span><strong style={kpiValueBlue}>{new Intl.NumberFormat('pt-BR',{style:'percent',minimumFractionDigits:2}).format(totais.percentual)}</strong></div>
                <div><span style={kpiLabel}>Ticket Médio</span><strong style={kpiValueBlue}>{new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL',maximumFractionDigits:0}).format(totais.ticketMedio)}</strong></div>
              </div>
              <div ref={kpisScrollRef} className="custom-scroll" onScroll={verificarScrollKpis} style={{display:'grid',gridTemplateColumns:'repeat(6,minmax(90px,1fr))',gap:'5px',textAlign:'center',overflowX:'auto',flex:1,paddingLeft:'8px',paddingRight:'8px'}}>
                <div><span style={kpiLabel}>Qtd Obras</span><strong style={kpiValue}> {totais.qtdObras}</strong></div>
                <div><span style={kpiLabel}>Postes</span><strong style={kpiValue}>{totais.poste}</strong></div>
                <div><span style={kpiLabel}>Rede MT (m)</span><strong style={kpiValue}>{totais.kmmt}</strong></div>
                <div><span style={kpiLabel}>Rede BT (m)</span><strong style={kpiValue}>{totais.kmbt}</strong></div>
                <div><span style={kpiLabel}>Trafos</span><strong style={kpiValue}>{totais.trafo}</strong></div>
                <div><span style={kpiLabel}>Clientes</span><strong style={kpiValue}>{totais.clientes}</strong></div>
              </div>
              <button onClick={()=>rolarKpis(1)} disabled={!podeRolarKpisDir} style={{...minimalScrollBtnStyle,display:temOverflowKpis?'flex':'none',marginLeft:'8px'}}><ChevronRight size={15}/></button>
            </div>
          </div>
          ...
          <div style={{background:'#fff',border:'1px solid #cbd5e1',borderRadius:'8px',display:'flex',flexDirection:'column',flex:1,minHeight:0,overflow:'hidden'}}>
            {loading && listaCarteiraGlobal.length===0 ? (
              <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#64748b",fontWeight:"500"}}>Atualizando dados<span className="loading-dots"></span></div>
            ) : (
              <DataTable
...
const kpiLabel = {display:'block',fontSize:'0.7rem',color:'#64748b'};
const kpiValueBlue = {fontSize:'0.85rem',color:'#005596'};
const kpiValue = {fontSize:'0.85rem',color:'#1e293b'};

const btnStyle = {
  background:'#fff',color:'#334151',border:'1px solid #cbd5e1',height:'32px',
  padding:'0 12px',minWidth:'100px',borderRadius:'6px',fontSize:'0.8rem',
  fontWeight:'600',display:'inline-flex',alignItems:'center',justifyContent:'center',
  gap:'6px',cursor:'pointer',boxSizing:'border-box',boxShadow:'0 1px 2px rgba(0,0,0,0.02)'
};

const minimalScrollBtnStyle = {
  background:'#ffffff',border:'1px solid #cbd5e1',borderRadius:'50%',width:'24px',
  height:'24px',display:'flex',alignItems:'center',justifyContent:'center',
  cursor:'pointer',color:'#475569',flexShrink:0,boxShadow:'0 2px 4px rgba(0,0,0,0.06)'
};